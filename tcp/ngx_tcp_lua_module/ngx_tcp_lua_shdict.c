...
static int ngx_tcp_lua_shdict_safe_set(lua_State *L);
static int ngx_tcp_lua_shdict_get(lua_State *L);
static int ngx_tcp_lua_shdict_get_stale(lua_State *L);
static int ngx_tcp_lua_shdict_get_helper(lua_State *L, int get_stale);
...
static int ngx_tcp_lua_shdict_safe_add(lua_State *L);
...
static int ngx_tcp_lua_shdict_get_keys(lua_State *L);


#define NGX_TCP_LUA_SHDICT_ADD         0x0001
#define NGX_TCP_LUA_SHDICT_REPLACE     0x0002
#define NGX_TCP_LUA_SHDICT_SAFE_STORE  0x0004
...
ngx_tcp_lua_inject_shdict_api(ngx_tcp_lua_main_conf_t *lmcf, lua_State *L)
{
    ...
    if (lmcf->shm_zones != NULL) {
        ...
        lua_pushcfunction(L, ngx_tcp_lua_shdict_get_keys);
...
{
  return ngx_tcp_lua_shdict_get_helper(L, 0 /* stale */);
}


static int
ngx_tcp_lua_shdict_get_stale(lua_State *L)
{
  return ngx_tcp_lua_shdict_get_helper(L, 1 /* stale */);
}


static int
ngx_tcp_lua_shdict_get_helper(lua_State *L, int get_stale)
{
    int                          n;
    ngx_str_t                    name;
    ngx_str_t                    key;
...
static int
ngx_tcp_lua_shdict_safe_add(lua_State *L)
{
    return ngx_tcp_lua_shdict_set_helper(L, NGX_TCP_LUA_SHDICT_ADD
                                          |NGX_TCP_LUA_SHDICT_SAFE_STORE);
}
...
static int
ngx_tcp_lua_shdict_safe_set(lua_State *L)
{
    return ngx_tcp_lua_shdict_set_helper(L, NGX_TCP_LUA_SHDICT_SAFE_STORE);
}
...
int
ngx_tcp_lua_ffi_shdict_get(ngx_shm_zone_t *zone, u_char *key,
    size_t key_len, int *value_type, u_char **str_value_buf,
...
#if (NGX_DEBUG)
    ngx_log_debug3(NGX_LOG_DEBUG_TCP, ctx->log, 0,
