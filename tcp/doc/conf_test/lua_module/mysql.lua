...
--number(>0) or nil
function _M.set_timeout(self, connect_timeout,send_timeout,read_timeout)
    local sock = self.sock
    if not sock then
        return nil, "not initialized"
    end

    return sock:settimeout(connect_timeout,send_timeout,read_timeout)
end
...
function _M.connect(self, opts)
    ...
    if host then
        ...
        ok, err = sock:connect(host, port, pool)
