 ...
 * */
static ngx_int_t
ngx_tcp_lua_cache_load_code(lua_State *L, const char *key)
{
 ...
 * */
static ngx_int_t
ngx_tcp_lua_cache_store_code(lua_State *L, const char *key)
{
    int rc;
        ...
        const u_char *cache_key, char **err, unsigned enabled)
{
    ...
    if (enabled) {
        if (cache_key == NULL) {
            ...
            p = ngx_copy(buf, NGX_TCP_LUA_FILE_TAG, NGX_TCP_LUA_FILE_TAG_LEN);

            p = ngx_tcp_lua_digest_hex(p, script, ngx_strlen(script));
        ...
        if (ngx_tcp_lua_cache_load_code(L, (char *) cache_key) == NGX_OK) {
    ...
    } else {    /*  call closure factory to generate new closure */
        rc = lua_pcall(L, 0, 1, 0);
        if (rc != 0) {
            dd("Error: failed to call closure factory!!");
            return NGX_ERROR;
