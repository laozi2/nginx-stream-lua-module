...
//#endif
    ...
    ngx_tcp_lua_complex_value_t    *replace;
...
typedef struct {
    ...
    int                      captures_len;
    uint8_t                  flags;
} ngx_tcp_lua_regex_ctx_t;


static int ngx_tcp_lua_ngx_re_gmatch_iterator(lua_State *L);
static ngx_uint_t ngx_tcp_lua_ngx_re_parse_opts(lua_State *L,
    ngx_tcp_lua_regex_compile_t *re, ngx_str_t *opts, int narg);
static int ngx_tcp_lua_ngx_re_sub_helper(lua_State *L, unsigned global);
static int ngx_tcp_lua_ngx_re_match_helper(lua_State *L, int wantcaps);
static int ngx_tcp_lua_ngx_re_find(lua_State *L);
static int ngx_tcp_lua_ngx_re_match(lua_State *L);
static int ngx_tcp_lua_ngx_re_gmatch(lua_State *L);
static int ngx_tcp_lua_ngx_re_sub(lua_State *L);
static int ngx_tcp_lua_ngx_re_gsub(lua_State *L);
static void ngx_tcp_lua_regex_free_study_data(ngx_pool_t *pool,
    pcre_extra *sd);
static ngx_int_t ngx_tcp_lua_regex_compile(ngx_tcp_lua_regex_compile_t *rc);
static void ngx_tcp_lua_ngx_re_gmatch_cleanup(void *data);
static int ngx_tcp_lua_ngx_re_gmatch_gc(lua_State *L);
static void ngx_tcp_lua_re_collect_named_captures(lua_State *L,
    int res_tb_idx, u_char *name_table, int name_count, int name_entry_size,
    unsigned flags, ngx_str_t *subj);


#define ngx_tcp_lua_regex_exec(re, e, s, start, captures, size, opts)       \
    pcre_exec(re, e, (const char *) (s)->data, (s)->len, start, opts,        \
              captures, size)


#define ngx_tcp_lua_regex_dfa_exec(re, e, s, start, captures, size, ws,     \
                                    wscount, opts)                           \
...
static int
ngx_tcp_lua_ngx_re_match(lua_State *L)
{
    return ngx_tcp_lua_ngx_re_match_helper(L, 1 /* want captures */);
}


static int
ngx_tcp_lua_ngx_re_find(lua_State *L)
{
    return ngx_tcp_lua_ngx_re_match_helper(L, 0 /* want captures */);
}


static int
ngx_tcp_lua_ngx_re_match_helper(lua_State *L, int wantcaps)
{
    /* u_char                      *p; */
    int                          res_tb_idx = 0;
    ngx_tcp_session_t           *s;
    ...
    ngx_tcp_lua_regex_compile_t      re_comp;
    ...
    ngx_memzero(&re_comp, sizeof(ngx_tcp_lua_regex_compile_t));
    ...
    flags = ngx_tcp_lua_ngx_re_parse_opts(L, &re_comp, &opts, 3);
    ...
    dd("pool %p, s pool %p", pool, s->pool);
    ...
    ngx_log_debug5(NGX_LOG_DEBUG_TCP, s->connection->log, 0,
    ...
    old_pool = ngx_tcp_lua_pcre_malloc_init(pool);

    rc = ngx_tcp_lua_regex_compile(&re_comp);
    ...
    if (flags & NGX_LUA_RE_COMPILE_ONCE) {
        ...
        re = ngx_palloc(pool, sizeof(ngx_tcp_lua_regex_t));
        if (re == NULL) {
            msg = "no memory";
            goto error;
        }
...
exec:
    ...
    if (flags & NGX_LUA_RE_MODE_DFA) {
        ...
        rc = ngx_tcp_lua_regex_dfa_exec(re_comp.regex, sd, &subj,
        ...
        rc = ngx_tcp_lua_regex_exec(re_comp.regex, sd, &subj, (int) pos, cap,
                                     ovecsize, exec_opts);
    }
    ...
    if (rc == NGX_REGEX_NO_MATCHED) {
        ...
        if (!(flags & NGX_LUA_RE_COMPILE_ONCE)) {
            if (sd) {
                ngx_tcp_lua_regex_free_study_data(pool, sd);
            }
    ...
    if (name_count > 0) {
        ngx_tcp_lua_re_collect_named_captures(L, res_tb_idx, name_table,
...
static int
ngx_tcp_lua_ngx_re_gmatch(lua_State *L)
{
    ngx_tcp_lua_main_conf_t     *lmcf;
    ngx_tcp_session_t           *s;
    ngx_str_t                    subj;
...
compiled:
    ...
    ctx = lua_newuserdata(L, sizeof(ngx_tcp_lua_regex_ctx_t));
    ...
    if (!(flags & NGX_LUA_RE_COMPILE_ONCE)) {
        ...
        lua_pushcfunction(L, ngx_tcp_lua_ngx_re_gmatch_gc);
...
//        }
//        
//        cln->handler = ngx_tcp_lua_ngx_re_gmatch_cleanup;
//        cln->data = ctx;
//        ctx->cleanup = &cln->handler;
        ctx->cleanup = ngx_tcp_lua_ngx_re_gmatch_cleanup;
    } else {
        ctx->cleanup = NULL;
    }
    ...
    lua_pushcclosure(L, ngx_tcp_lua_ngx_re_gmatch_iterator, 3);
...
static int
ngx_tcp_lua_ngx_re_gmatch_iterator(lua_State *L)
{
    ngx_tcp_lua_regex_ctx_t     *ctx;
    ngx_tcp_session_t           *s;
    int                         *cap;
    ...
    ctx = (ngx_tcp_lua_regex_ctx_t *) lua_touserdata(L, lua_upvalueindex(2));
    ...
    dd("offset %d, s %p, subj %s", (int) offset, ctx->request, subj.data);
    ...
    if (s != ctx->request || s->pool != ctx->request->pool) {
    ...
    if (ctx->flags & NGX_LUA_RE_MODE_DFA) {
        ...
        rc = ngx_tcp_lua_regex_dfa_exec(ctx->regex, ctx->regex_sd, &subj,
        ...
        rc = ngx_tcp_lua_regex_exec(ctx->regex, ctx->regex_sd, &subj,
    ...
    if (rc == NGX_REGEX_NO_MATCHED) {
        ...
        if (!(ctx->flags & NGX_LUA_RE_COMPILE_ONCE)) {
            if (ctx->regex_sd) {
                ngx_tcp_lua_regex_free_study_data(s->pool, ctx->regex_sd);
                ctx->regex_sd = NULL;
            }

            ngx_pfree(s->pool, cap);
        }
    ...
    if (name_count > 0) {
        ngx_tcp_lua_re_collect_named_captures(L, lua_gettop(L), name_table,
                                               name_count, name_entry_size,
                                               ctx->flags, &subj);
    }
...
static ngx_uint_t
ngx_tcp_lua_ngx_re_parse_opts(lua_State *L, ngx_tcp_lua_regex_compile_t *re,
...
static int
ngx_tcp_lua_ngx_re_sub(lua_State *L)
{
    return ngx_tcp_lua_ngx_re_sub_helper(L, 0 /* global */);
}


static int
ngx_tcp_lua_ngx_re_gsub(lua_State *L)
{
    return ngx_tcp_lua_ngx_re_sub_helper(L, 1 /* global */);
}


static int
ngx_tcp_lua_ngx_re_sub_helper(lua_State *L, unsigned global)
{
    ngx_tcp_lua_regex_t         *re;
    ngx_tcp_session_t           *s;
    ngx_str_t                    subj;
    ...
    ngx_tcp_lua_regex_compile_t               re_comp;
    ngx_tcp_lua_complex_value_t              *ctpl = NULL;
    ngx_tcp_lua_compile_complex_value_t       ccv;
    ...
    flags = ngx_tcp_lua_ngx_re_parse_opts(L, &re_comp, &opts, 4);
    ...
    ngx_log_debug6(NGX_LOG_DEBUG_TCP, s->connection->log, 0,
    ...
    if (func) {
        ...
        ctpl = ngx_palloc(pool, sizeof(ngx_tcp_lua_complex_value_t));
        if (ctpl == NULL) {
            flags &= ~NGX_LUA_RE_COMPILE_ONCE;
            msg = "no memory";
            goto error;
        ...
        ngx_memzero(&ccv, sizeof(ngx_tcp_lua_compile_complex_value_t));
        ccv.pool = pool;
        ccv.log = s->connection->log;
        ccv.value = &tpl;
        ccv.complex_value = ctpl;

        if (ngx_tcp_lua_compile_complex_value(&ccv) != NGX_OK) {
            ngx_pfree(pool, cap);
            ngx_pfree(pool, ctpl);
...
exec:
    ...
    for (;;) {
        if (flags & NGX_LUA_RE_MODE_DFA) {
            ...
            rc = ngx_tcp_lua_regex_exec(re_comp.regex, sd, &subj, offset, cap,
                                         ovecsize, exec_opts);
        }
        ...
        if (func) {
            ...
            if (name_count > 0) {
                ngx_tcp_lua_re_collect_named_captures(L, lua_gettop(L),
                                                       name_table,
                                                       name_count,
                                                       name_entry_size,
                                                       flags, &subj);
        ...
        rc = ngx_tcp_lua_complex_value(s, &subj, cp_offset, rc, cap, ctpl,
                &luabuf);
...
void
ngx_tcp_lua_inject_regex_api(lua_State *L)
{
    /* ngx.re */

    lua_createtable(L, 0, 5 /* nrec */);    /* .re */

    lua_pushcfunction(L, ngx_tcp_lua_ngx_re_find);
    lua_setfield(L, -2, "find");

    lua_pushcfunction(L, ngx_tcp_lua_ngx_re_match);
    lua_setfield(L, -2, "match");

    lua_pushcfunction(L, ngx_tcp_lua_ngx_re_gmatch);
    lua_setfield(L, -2, "gmatch");

    lua_pushcfunction(L, ngx_tcp_lua_ngx_re_sub);
    lua_setfield(L, -2, "sub");

    lua_pushcfunction(L, ngx_tcp_lua_ngx_re_gsub);
...
static void
ngx_tcp_lua_regex_free_study_data(ngx_pool_t *pool, pcre_extra *sd)
{
    ngx_pool_t              *old_pool;
...
static ngx_int_t
ngx_tcp_lua_regex_compile(ngx_tcp_lua_regex_compile_t *rc)
{
    int           n, erroff;
    char         *p;
    const char   *errstr;
    ...
    old_pool = ngx_tcp_lua_pcre_malloc_init(rc->pool);
...
static void
ngx_tcp_lua_ngx_re_gmatch_cleanup(void *data)
{
    ngx_tcp_lua_regex_ctx_t    *ctx = data;

    if (ctx) {
        if (ctx->regex_sd) {
            ngx_tcp_lua_regex_free_study_data(ctx->request->pool,
                                               ctx->regex_sd);
            ctx->regex_sd = NULL;
        }
...
static int
ngx_tcp_lua_ngx_re_gmatch_gc(lua_State *L)
{
    ngx_tcp_lua_regex_ctx_t    *ctx;
    ...
    if (ctx && ctx->cleanup) {
        ngx_tcp_lua_ngx_re_gmatch_cleanup(ctx);
    }
...
static void
ngx_tcp_lua_re_collect_named_captures(lua_State *L, int res_tb_idx,
