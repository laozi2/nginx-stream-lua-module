...
function _M.init(self, opts)
    ...
    self.max_head_size = opts.max_head_size or DEAFULT_MAX_HEAD_SIZE
    self.max_body_size = opts.max_body_size or DEAFULT_MAX_BODY_SIZE
    return 1,"ok"
end
...
function _M.send_request(self, req_tb)
    local sock = self.sock
    if not sock then
        return nil, "not initialized"
    end
...
function _M.receive_response(self)
...
function _M.http_request(self, req_tb)
    local start = ngx.now()
